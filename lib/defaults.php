...
add_action( 'people_create_metaboxes', function() {
	add_meta_box( 'brief', __( 'Brief Bio', 'people' ), 'render_people_brief_metabox', 'people', 'normal', 'high' );
} );

/**
 * Render people brief bio meta box.
 * 
 * Calls 'people_brief_metabox_render' action.
 */
if ( ! function_exists('render_people_brief_metabox') ) {
	function render_people_brief_metabox( $post ) {
		add_action( 'people_brief_metabox_render' , 
			function( $post ) {
				wp_nonce_field( 'people', 'people_brief_nonce' );
				people_metabox_basic_tinymce( $post, 'brief', '150px' );
			}
		);
		do_action( 'people_brief_metabox_render', $post );
	}
}
// save action
add_action('save_post',
	function( $post_id ) {
		people_save_meta( $post_id, 'people', 'people_brief_nonce', 'brief' );
	}
);

// Add people_person hook
add_filter( 'people_atts', 
	function( $arr, $id ) {
		$value = get_post_meta( $id, '_brief', true );
		if ( ! $value ) 
